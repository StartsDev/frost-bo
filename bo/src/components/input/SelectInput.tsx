...
export const SelectInput = ({data, handleChange, label, name, placeholder, selected, value, property, styleInput}: Props) => {
  return (
    <div className={styles.selectContainer}>
      ...
      <select name={name} id="" className={styles.formInput} style={styleInput} onChange={(e)=> handleChange(e as never)}>
        ...
        {data?.map((e: any , index: number) =>(
          <option key={index} value={e[value]} selected={selected}>{e[property]}</option>
        ))}
